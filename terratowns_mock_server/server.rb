...
class Home
  ...
  # we want to lock this down to only be from cloudfront.net 
  validates :domain_name, 
    format: { with: /\.cloudfront\.net\z/, message: "domain must be from .cloudfront.net" }
    # uniqueness: true, 

  # content version has to be an integer
  # we will (andrew) make sure it is an incremental version in the controller
...
class TerraTownsMockServer < Sinatra::Base
  ...
  #  Return an hard-coded token. Pretending it cames from the database
  def x_access_code
  ...
  def find_user_by_bearer_token
    ...
    # Split the token from the bearer word and check if the token exists in our database.
    # If we can't find it or doesn't match, then return an 401 error
    ...
    # the .params will grab the information from thequerystring in the url
